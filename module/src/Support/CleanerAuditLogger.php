...
class CleanerAuditLogger
{
    ...
    public function record(AddonModule $module, string $action, string $status, array $details, ?User $actor = null): void
    {
        ...
        DB::table('module_cleaner_cleanup_logs')->insert([
            ...
            'navigation_planned' => json_encode($this->targetsForSurface($details, 'navigation'), JSON_THROW_ON_ERROR),
            'storage_planned' => json_encode($this->targetsForSurface($details, 'storage'), JSON_THROW_ON_ERROR),
            'packages_planned' => json_encode($this->targetsForSurface($details, 'packages'), JSON_THROW_ON_ERROR),
            'module_files_planned' => json_encode($this->targetsForSurface($details, 'module_files'), JSON_THROW_ON_ERROR),
